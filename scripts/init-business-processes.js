...
/**
 * Скрипт для инициализации бизнес-процессов
 * Заполняет базу данных начальными бизнес-процессами
 */
...
/**
 * Создает начальные бизнес-процессы
 */
async function initBusinessProcesses() {
  console.log('Инициализация бизнес-процессов...');
  ...
  try {
    ...
    if (count > 0) {
      console.log(`В базе уже есть ${count} бизнес-процессов. Пропускаем инициализацию.`);
      return;
    }
    ...
    console.log('Бизнес-процессы успешно инициализированы!');
  } catch (error) {
    console.error('Ошибка при инициализации бизнес-процессов:', error);
  } finally {
